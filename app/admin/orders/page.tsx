...
interface AdminOrder {
  ...
  total: number; commission: number; status: OrderStatus; date: string; tracking?: string;
}
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: "0.35rem", marginBottom: "1.25rem", flexWrap: "wrap", alignItems: "center" }}>
        {STATUS_TABS.map(t => (
          <button key={t.value} onClick={() => setStatusFilter(t.value)} style={{
            ...
            background: statusFilter === t.value ? A.accent : A.card,
            color: statusFilter === t.value ? "#fff" : A.muted,
            border: `1px solid ${statusFilter === t.value ? A.accent : A.border}`, transition: "all 0.15s" }}>
            {t.label}
          </button>
        ))}
        <input type="text" placeholder="Search order, buyer, seller…" value={search} onChange={e => setSearch(e.target.value)}
      ...
      <div style={{ background: A.card, border: `1px solid ${A.border}`, overflowX: "auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "70px 1fr 130px 90px 90px 100px 110px",
        ...
        {filtered.map((o, i) => {
          ...
          return (
            <div key={o.id} style={{
              ...
              <span className="tabular-nums" style={{ fontFamily: "var(--font-jost)", fontWeight: 600, fontSize: "0.78rem", color: "#065F46" }}>
                +${o.commission}
              </span>
