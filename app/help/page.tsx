...
const SECTIONS = [
  ...
  { id: "disputes",   label: "Dispute Resolution", icon: "⚖" },
...
const FAQ = [
  ...
  },
  {
    ...
    a: "Yes. If the item arrived significantly not as described (wrong item, missing pieces, clear misrepresentation of size, color, or condition), report it through Veeral within 7 days of confirmed delivery with photos. Minor pre-owned wear and fit issues when accurate measurements were provided are not covered.",
  },
  {
    q: "How do I refund a buyer?",
    a: "Refunds for covered disputes are handled through Veeral Support, not directly between sellers and buyers. If a dispute is opened, Veeral will review the case and may issue a refund to the buyer if the claim is valid. Your payout will be placed on hold during the review.",
  },
  {
    q: "Will I have to pay tax or customs on my order?",
    a: "Buyers in the US are responsible for any applicable sales tax. International buyers may be subject to customs duties or import taxes — these are not included in Veeral's prices and are the buyer's responsibility. Sellers are responsible for reporting income taxes on their Veeral earnings.",
  },
];
...
function H2({ children }: { children: React.ReactNode }) {
  return (
    <h2 style={{ fontFamily: "var(--font-cormorant)", fontStyle: "italic", fontWeight: 400, fontSize: "clamp(1.5rem, 2.5vw, 1.9rem)", color: "#1A1A18", marginBottom: "1rem", lineHeight: 1.15 }}>
      {children}
    </h2>
  );
}
...
function H3({ children }: { children: React.ReactNode }) {
  return (
    <h3 style={{ fontFamily: "var(--font-jost)", fontWeight: 700, fontSize: "0.82rem", letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--burnt-orange)", marginTop: "1.75rem", marginBottom: "0.6rem" }}>
      {children}
    </h3>
  );
}
...
function P({ children }: { children: React.ReactNode }) {
  return (
    <p style={{ fontFamily: "var(--font-jost)", fontSize: "0.92rem", color: "var(--muted)", lineHeight: 1.85, marginBottom: "0.85rem" }}>
      {children}
    </p>
  );
}
...
function Ul({ items }: { items: string[] }) {
  return (
    <ul style={{ margin: "0 0 0.85rem 0", padding: "0 0 0 1.2rem" }}>
      {items.map((item, i) => (
        <li key={i} style={{ fontFamily: "var(--font-jost)", fontSize: "0.92rem", color: "var(--muted)", lineHeight: 1.85, marginBottom: "0.25rem" }}>
          {item}
        </li>
      ))}
    </ul>
...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid var(--warm-tan)" }}>
      <button
        onClick={() => setOpen(v => !v)}
        style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1.1rem 0", background: "none", border: "none", cursor: "pointer", textAlign: "left", gap: "1rem" }}
      >
        <span style={{ fontFamily: "var(--font-jost)", fontWeight: 600, fontSize: "0.9rem", color: "#1A1A18", lineHeight: 1.4 }}>
          {q}
        </span>
        <span style={{ fontFamily: "var(--font-jost)", fontSize: "1.1rem", color: "var(--burnt-orange)", flexShrink: 0, transition: "transform 0.2s", transform: open ? "rotate(45deg)" : "rotate(0deg)" }}>
          +
        </span>
      </button>
      {open && (
        <div style={{ paddingBottom: "1.1rem" }}>
          <p style={{ fontFamily: "var(--font-jost)", fontSize: "0.88rem", color: "var(--muted)", lineHeight: 1.8, margin: 0 }}>
            {a}
          </p>
        </div>
      )}
...
export default function HelpCenterPage() {
  ...
  return (
    <div style={{ background: "var(--cream)", minHeight: "100vh" }}>
      ...
      {/* Hero */}
      <div style={{ background: "#1A1A18", padding: "4rem 1.5rem 3rem", textAlign: "center" }}>
        <p style={{ fontFamily: "var(--font-jost)", fontWeight: 600, fontSize: "0.72rem", letterSpacing: "0.28em", textTransform: "uppercase", color: "var(--burnt-orange)", marginBottom: "0.75rem" }}>
          Help Center
        </p>
        <h1 style={{ fontFamily: "var(--font-cormorant)", fontStyle: "italic", fontWeight: 400, fontSize: "clamp(2rem, 5vw, 3rem)", color: "var(--cream)", lineHeight: 1.1, marginBottom: "0.75rem" }}>
          How can we help?
        </h1>
        <p style={{ fontFamily: "var(--font-jost)", fontSize: "0.88rem", color: "rgba(237,230,222,0.65)", maxWidth: "420px", margin: "0 auto 2rem", lineHeight: 1.75 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", justifyContent: "center", maxWidth: "700px", margin: "0 auto" }}>
          {SECTIONS.map(s => (
            <button key={s.id} onClick={() => scrollTo(s.id)} style={{ fontFamily: "var(--font-jost)", fontWeight: 600, fontSize: "0.72rem", letterSpacing: "0.14em", textTransform: "uppercase", padding: "0.4rem 0.85rem", background: "transparent", color: "rgba(237,230,222,0.75)", border: "1px solid rgba(237,230,222,0.2)", cursor: "pointer", transition: "all 0.15s" }}
              onMouseOver={e => { e.currentTarget.style.color = "var(--cream)"; e.currentTarget.style.borderColor = "rgba(237,230,222,0.5)"; }}
              onMouseOut={e => { e.currentTarget.style.color = "rgba(237,230,222,0.75)"; e.currentTarget.style.borderColor = "rgba(237,230,222,0.2)"; }}
            >
              {s.label}
            </button>
          ))}
      ...
      <div className="max-w-7xl mx-auto" style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 0, padding: "0", alignItems: "start" }}>

        {/* Sticky sidebar */}
        <aside style={{ position: "sticky", top: "0", alignSelf: "flex-start", borderRight: "1px solid var(--warm-tan)", padding: "2.5rem 0", minHeight: "100vh" }}>
          <p style={{ fontFamily: "var(--font-jost)", fontWeight: 700, fontSize: "0.7rem", letterSpacing: "0.25em", textTransform: "uppercase", color: "var(--burnt-orange)", padding: "0 1.5rem", marginBottom: "1rem" }}>
            Contents
          </p>
          <nav>
            {SECTIONS.map(s => (
              <button key={s.id} onClick={() => scrollTo(s.id)} style={{ display: "flex", alignItems: "center", gap: "0.6rem", width: "100%", padding: "0.6rem 1.5rem", background: activeSection === s.id ? "rgba(201,92,26,0.07)" : "transparent", borderRight: activeSection === s.id ? "2px solid var(--burnt-orange)" : "2px solid transparent", border: "none", borderLeft: "none", cursor: "pointer", textAlign: "left", transition: "all 0.15s" }}>
                <span style={{ fontSize: "0.85rem", opacity: 0.7, width: "16px", flexShrink: 0 }}>{s.icon}</span>
                <span style={{ fontFamily: "var(--font-jost)", fontSize: "0.8rem", fontWeight: activeSection === s.id ? 600 : 400, color: activeSection === s.id ? "var(--burnt-orange)" : "var(--muted)", letterSpacing: "0.02em" }}>
                  {s.label}
                </span>
              </button>
            ))}
        ...
        {/* Content */}
        <main style={{ padding: "3rem 3rem 5rem", maxWidth: "720px" }}>
          ...
          {/* Contact CTA */}
          <div style={{ marginTop: "3rem", padding: "2rem", background: "#fff", border: "1px solid var(--warm-tan)", textAlign: "center" }}>
            <p style={{ fontFamily: "var(--font-cormorant)", fontStyle: "italic", fontSize: "1.5rem", color: "#1A1A18", marginBottom: "0.4rem" }}>
              Still have a question?
            </p>
            <p style={{ fontFamily: "var(--font-jost)", fontSize: "0.85rem", color: "var(--muted)", opacity: 0.7, marginBottom: "1.25rem" }}>
              Our support team is here to help with orders, rentals, and anything else.
            </p>
            <Link href="/account/messages" style={{ fontFamily: "var(--font-jost)", fontWeight: 600, fontSize: "0.78rem", letterSpacing: "0.18em", textTransform: "uppercase", padding: "0.7rem 1.6rem", background: "var(--burnt-orange)", color: "var(--cream)", textDecoration: "none", display: "inline-block" }}>
              Contact Support
            </Link>
          </div>
      ...
      <style>{`
        @media (max-width: 768px) {
          .help-body { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </div>
  );
