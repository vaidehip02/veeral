...
const CONTENT_SECTIONS: ContentSection[] = [
  ...
  },
  {
    ...
    items: [
      { title: "Shipping Requirements for Sellers", body: <><Ul items={["Ship the item within 7 days of purchase","Use a tracked shipping service (USPS, UPS, or FedEx)","Upload the tracking number in your seller dashboard once the item is shipped"]} /><P>Tracking confirms that the item has been shipped and is required for your payout to process. Veeral does not currently receive delivery confirmation from carriers — tracking proves shipment, not arrival.</P></> },
      { title: "Where do I upload the tracking number?", body: <P>Go to your Seller Dashboard → Orders, find the relevant order, and click &ldquo;Mark as shipped.&rdquo; You&apos;ll be prompted to enter the tracking number there. This is required before your payout can process.</P> },
      { title: "Who pays for shipping? Do I need to buy a label?", body: <><P>The buyer pays a shipping fee at checkout. The amount depends on the tier the seller chose when listing: small ($8), medium ($14), large ($24), or a custom amount between $5–$45. Sellers may also offer free shipping, in which case no fee is charged to the buyer and the seller covers the label cost out of pocket.</P><P>Whatever amount is charged is included in your payout — it is passed through to you to cover your label. You are responsible for purchasing your own shipping label through USPS, UPS, or FedEx.</P><P>There is no in-app label purchase. You buy the label directly from the carrier and enter the tracking number in your seller dashboard.</P><P><strong>Note for heavy items:</strong> The large tier ($24) may not cover all lehenga sets or similarly heavy bridal pieces, which typically cost $24–$35 to ship. If you are listing such an item, use a custom amount that reflects the actual label cost.</P></> },
      { title: "What if I can&apos;t ship within 7 days?", body: <P>If you need more time, contact Veeral Support as soon as possible before the 7-day window closes. Do not let the window pass without communicating — buyers can open a dispute if an item is not shipped within 7 days of purchase, and this may affect your ability to sell on Veeral.</P> },
      { title: "What if the item is lost or damaged in transit?", body: <><P>If a buyer reports that an item was lost or damaged in transit, Veeral&apos;s Buyer Protection covers them — we will issue a refund. Your payout is not clawed back in this case; the transfer to your account has already been made at the time of purchase.</P><P>To protect yourself: always use a tracked carrier, and consider purchasing shipping insurance for high-value items. Veeral strongly recommends this for garments over $500.</P></> },
      { title: "Preparing Your Item for Shipment", body: <><P>Indian garments often include delicate fabrics and detailed handwork. Careful packaging matters.</P><Ul items={["Fold garments gently to protect embroidery or beadwork","Use protective inner wrapping when possible","Remove or cover old shipping labels if reusing packaging","Choose packaging that fits closely to reduce movement and waste","If your listing includes multiple pieces (e.g., blouse, dupatta, and petticoat), pack and count each piece before sealing the box — missing pieces are one of the most common buyer complaints"]} /></> },
    ],
  },
  {
