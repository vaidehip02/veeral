...
export default function HeroCarousel() {
  ...
  useEffect(() => {
    fetch("/api/admin/hero-slides")
      .then(r => r.json())
      .then((data: Slide[]) => {
        const active = data.filter(s => s.active).sort((a, b) => a.order_index - b.order_index);
    ...
    >
      ...
      >
        ...
        <div className="flex-1 pr-8">
          ...
          <Link
            href={slide.href}
            style={{ fontFamily: "var(--font-jost)", fontWeight: 600, fontSize: "0.6rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--cream)", background: "var(--burnt-orange)", padding: "0.8rem 2rem", display: "inline-block", transition: "opacity 0.2s" }}
      ...
      {(["prev", "next"] as const).map((dir) => (
        <button key={dir} onClick={dir === "prev" ? prev : next} aria-label={dir === "prev" ? "Previous" : "Next"}
          style={{ position: "absolute", top: "50%", transform: "translateY(-50%)", [dir === "prev" ? "left" : "right"]: "1.2rem", width: "36px", height: "36px", borderRadius: "50%", background: "var(--cream)", border: "1px solid var(--warm-tan)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.9rem", color: "var(--dark)", transition: "background 0.2s", zIndex: 10 }}
